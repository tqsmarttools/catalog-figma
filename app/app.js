(function () {
  const STORAGE_KEY = "tqsmarttools-catalog-state-v1";
  ...
  const filterOptions = [
    { id: "all", label: "Tất cả" },
    { id: "can-go", label: "Bay cán gỗ" },
    { id: "can-tre", label: "Bay cán tre" },
  ...
  function addProduct(productId) {
    const nextQty = state.quote[productId] ? state.quote[productId] + 1 : 1;
    state.quote[productId] = nextQty;
    saveState();
    render();
  }
  ...
  function filteredProducts() {
    return products.filter((product) => {
      ...
      return categoryMatch && groupMatch && filterMatch && product.id !== "BXD-T01" && product.id !== "KCB-01" && product.id !== "BKH-60"
        ? true
        : categoryMatch && groupMatch && filterMatch && product.id.startsWith("BXD");
    }).slice(0, 6);
  }
